...
global magnitude
global filtered_magnitude
global steps_indicies, ___
global last_val
global count
...
def recv_data():
    ...
    while True:
        try:
            ...
            sys.stdout.flush()
            detectSteps(tvals, xvals, yvals, zvals)
...
def detectSteps(tvals, xvals, yvals, zvals):
    """
    ...
    # CALCULATE MAGNITUDE
    magnitude = np.sqrt(xvals ** 2 + yvals ** 2 + zvals ** 2)
    ...
    # COUNTING STEPS
    height_filter = 10.0001
    steps_indicies, ___ = find_peaks(filtered_magnitude, height=height_filter, prominence=.5)

    if (len(steps_indicies) > last_val):
        count += 1
        print("Step counted! Step count:" + str(count))
    last_val = len(steps_indicies)
...
def animate(i):
    ...
    try:
        ...
        ax2.plot(tvals, filtered_magnitude, 'g-', label = "magnitude")
        ax2.plot(tvals[steps_indicies], filtered_magnitude[steps_indicies], 'r*')
