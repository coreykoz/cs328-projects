...
class FeatureExtractor():
    ...
    def _compute_formant_features(self, window):
        ...
        result = np.histogram(frequencies, bins=20, range=(0, 5500))
    ...
    def _compute_delta(self, window, n):
        n_frames, n_coeffs = window.shape
        ...
        deltas = np.zeros((n_frames - 2 * n, n_coeffs))

        for t in range(n, n_frames - n):
            deltas[t - n] = np.sum(float(n) * window[t + 1:t + n + 1], axis=0) - np.sum(float(n) * window[t - n:t], axis=0)
    ...
    def _compute_delta_coefficients(self, window, n=2):
        ...
        delta_feat = delta(mfccs, n)

        delta_feat = np.delete(delta_feat, 78, 0)
        delta_feat = np.delete(delta_feat, 77, 0)
        delta_feat = np.delete(delta_feat, 1, 0)
        delta_feat = np.delete(delta_feat, 0, 0)

        return np.ndarray.flatten(delta_feat)

    ''' Tried adding another feature, didn't help accuracy
    
    def _compute_delta_deltas(self, window, n=2):
        mfccs = self._compute_mfcc(window)
        delta_feat = delta(mfccs, n)
        delta_delta = delta(delta_feat, n)

        delta_delta = np.delete(delta_delta, 78, 0)
        delta_delta = np.delete(delta_delta, 77, 0)
        delta_delta = np.delete(delta_delta, 1, 0)
        delta_delta = np.delete(delta_delta, 0, 0)

        return np.ndarray.flatten(delta_delta)
    ...
    def extract_features(self, window, debug=True):
        ...
        #x = np.append(x, self._compute_delta_deltas(window))
        return x    
